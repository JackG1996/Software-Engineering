...
public class BinaryTreeTest 
{
	 ...
	 public void testLca()
	{
		...
		assertEquals("Checking the LCA of right and left node","7", BST.lca(8,3)); // checks that root is the lca 
	...
	public void testPut()
	{
		...
		assertNull("Checking the LCA of a null tree with no children", BST.lca(7,7));
		...
		BST.put(null, 1);
		assertEquals("Checking when null key and valid value inserted", 0, BST.size());
		
		BST.put(1, null);
		assertEquals("Checking when valid key and null value inserted", 0, BST.size());

		BST.put(7, 7);   //        _7_
        BST.put(7, 8);   //        _8_
        assertEquals("Check if code properly insert nodes based upon key", 1, BST.size()); // size is one and not 2 despite two puts as keys are the same and the value is only updated
	...
	public void testLcaDelete() {
		...
		BST.put(7, 9); // _9_
		BST.delete(7); // must delete key and value before updating the tree with the same value for a different key
		BST.put(8, 9); // _9_
		assertEquals("Check if code properly registers that the same/new value has been inputted to new key", 1,BST.size()); // size is one and not 2 despite two puts as value is the same.
		...
		assertEquals("Checking the LCA of right node and left node","7", BST.lca(8,3));
		BST.delete(3);
		assertEquals("Checking the LCA of right node and left node in subtree after deletion","7", BST.lca(8,1));
		
		assertEquals("Checking the LCA of right node and right node","8", BST.lca(8,12));
		BST.delete(12);
		assertNull("Checking the LCA of right node and right node after deletion",BST.lca(8,12));
